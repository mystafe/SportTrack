        ...
        const process = (deadline?: IdleDeadline) => {
          ...
          while (i < records.length && (!deadline || deadline.timeRemaining() > 0)) {
            ...
            try {
              ...
              errors.push(
                `Record ${i + 1}: ${error instanceof Error ? error.message : 'Unknown error'}`
              );
            }
  ...
  try {
    ...
    if (sizeMB > 1000) {
      ...
      for (let offset = 0; offset < arrayBuffer.byteLength; offset += chunkSize) {
        const chunk = arrayBuffer.slice(
          offset,
          Math.min(offset + chunkSize, arrayBuffer.byteLength)
        );
        const chunkText = decoder.decode(chunk, {
          stream: offset + chunkSize < arrayBuffer.byteLength,
        });
        text += chunkText;
    ...
    const text = await new Promise<string | ArrayBuffer>((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        try {
          ...
          if (errorMessage.includes('length') || errorMessage.includes('Invalid string')) {
            reject(
              new Error(
                `File is too large to process as text (${Math.round(sizeMB)}MB). Please try exporting a smaller date range from Apple Health.`
              )
            );
          } else {
      ...
      reader.onerror = () => {
        if (timeoutId) clearTimeout(timeoutId);
        reject(
          new Error('File reading error. The file may be corrupted or too large for your browser.')
        );
      };
      ...
      if (sizeMB > 500) {
        timeoutId = setTimeout(
          () => {
            ...
            reject(
              new Error(
                `File reading timeout. The file (${Math.round(sizeMB)}MB) is too large to process. Please try splitting it or using a smaller date range.`
              )
            );
          },
          5 * 60 * 1000
      ...
      try {
        if (text instanceof ArrayBuffer) {
          ...
          if (sizeMB > 50) {
            ...
            for (let offset = 0; offset < text.byteLength; offset += chunkSize) {
              ...
              const chunkText = decoder.decode(chunk, {
                stream: offset + chunkSize < text.byteLength,
              });
        ...
        if (
          ...
          return {
            ...
            data: [],
            errors: [
              `CSV file is too large to process (${Math.round(sizeMB)}MB). Please try exporting a smaller date range from Apple Health, or use XML format instead.`,
            ],
            totalRecords: 0,
            dateRange: null,
          };
    ...
    if (errorMessage.includes('memory') || errorMessage.includes('quota')) {
      return {
        ...
        data: [],
        errors: [
          `File is too large for your browser to process (${Math.round(sizeMB)}MB). Try using a smaller date range or split the file.`,
        ],
        totalRecords: 0,
        dateRange: null,
      };
