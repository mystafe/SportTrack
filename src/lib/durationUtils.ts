 ...
 */
export function calculateDurationStats(activities: ActivityRecord[]): DurationStats {
  ...
  for (const activity of activities) {
    if (activity.duration && activity.duration > 0) {
      ...
      dailyDurations.set(dayKey, (dailyDurations.get(dayKey) ?? 0) + activity.duration);
    }
  }
  ...
  const averageDailyDuration =
    daysWithDuration > 0 ? Math.round(totalDuration / daysWithDuration) : 0;
