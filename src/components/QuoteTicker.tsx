  ...
  return (
    ...
    >
      <div className="relative w-full h-full flex items-center justify-center min-h-[2.5rem] sm:min-h-[2.75rem]">
        ...
        <div
          className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-brand/80 dark:bg-brand/90 z-10 animate-pulse shadow-lg"
          ...
          <div className="quote-ticker-content flex items-center h-full">
            {Array.from({ length: 6 }).map((_, i) => (
              <span key={i} className="inline-flex items-center whitespace-nowrap h-full">
                ...
                {i < 5 && (
                  <span className="mx-6 sm:mx-8 opacity-70 text-brand dark:text-brand-light text-lg flex items-center h-full">
                    ✦
                  </span>
                )}
              </span>
