    ...
    <div className="spacing-lg">
      ...
      {recordsByType.overall.length > 0 && (
        <div>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2 lg:grid-cols-3'} spacing-sm`}>
            {recordsByType.overall.map(record => (
              ...
              >
                ...
                <div className={`${isMobile ? 'text-base' : 'text-lg'} font-bold text-brand dark:text-brand-light mb-1`}>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2 lg:grid-cols-3'} spacing-sm`}>
            {recordsByType.activities.map(record => (
              ...
              >
                ...
                <div className={`${isMobile ? 'text-sm' : 'text-base'} font-bold text-brand dark:text-brand-light mb-1`}>
                  {formatRecordValue(record, lang)}
                </div>
                <div className="text-body-small text-gray-600 dark:text-gray-400">
                  {format(parseISO(record.date), 'dd MMM yyyy', { locale: dateLocale })}
